...
const Reconsilation = () => {
    ...
    useEffect(() => {
        axios.get(`https://www.api.textilediwanji.com/reconsilecompany/${id}`)
            .then(res => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response1 = await axios.get(`https://www.api.textilediwanji.com/reconsilation/${id}`);
                const response2 = await axios.get(`https://www.api.textilediwanji.com/packslip/${id}/${id1}`);
                const response3 = await axios.get(`https://www.api.textilediwanji.com/yarninward/${id}`);
                const response4 = await axios.get(`https://www.api.textilediwanji.com/billingreport/${id}/${id1}`);
    ...
    useEffect(() => {
        axios.get(`https://www.api.textilediwanji.com/reconsilationbeaminward/${id}/${id1}`)
            .then(res => {
                    ...
                    <div className="container border border-1 ">
                        ...
                        <div className="row">
                            <div className="col-2 border border-1 d-flex justify-content-center align-items-center ">
                                <img src={`https://www.api.textilediwanji.com/companyimage/${company.filenameas}`} style={{ maxWidth: 250, maxHeight: 90 }} alt={`Image ${company.filenameas}`} />
                            </div>
                            <div className="col-10 border border-1">
                                {company && <h3 className="m-0">{company.companyname}</h3>}
                                {company && <p className="m-0">{company.companyaddress}</p>}
                                {company && <p className="m-0"> Email id:-{company.emailid} phone:{company.phoneno}</p>}
                                {company && <p className="m-0">GST No:- {company.gst}</p>}
                            </div>
                        </div>
                        ...
                        <div className="row d-flex ">
                            ...
                            <div className="col-6 border border-1">
                                ...
                                <div className="row mt-5 mb-4 me-3 d-flex justify-content-end">
                                    <QRCode className="mt-5" value={`http://192.168.157.13:3000/reconsilation/${id}`}></QRCode>
