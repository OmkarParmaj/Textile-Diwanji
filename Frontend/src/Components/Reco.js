...
const Reco = () => {
    ...
    const designno = query.get('designno');
    const recoemail = query.get('recoemail');
    // const emailid = query.get('emailid');


    useEffect(() => {
        axios.get(`http://www.textilediwanji.com:4000/reconsilecompany?setno=${setno}&designno=${designno}&recoemail=${recoemail}`)
            .then(res => {
                // console.log(res.data);
                setCompany(res.data[0]);
                const middd = res.data[0].emailid;
               
                setUrl(`http://www.textilediwanji.com/rec?setno=${setno}&designno=${designno}&recoemail=${recoemail}`);
                setMailid(middd);
            })
            .catch(err => {
                // console.log(err);
    ...
    useEffect(() => {
        ...
        axios.post(`http://www.textilediwanji.com:4000/packinginfo`, { mailid })
        .then(res => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response1 = await axios.get(`http://www.textilediwanji.com:4000/reconsilation?setno=${setno}&designno=${designno}&recoemail=${recoemail}`);
                const response2 = await axios.get(`http://www.textilediwanji.com:4000/packslip?setno=${setno}&designno=${designno}&recoemail=${recoemail}`);
                const response3 = await axios.get(`http://www.textilediwanji.com:4000/yarninward?setno=${setno}&designno=${designno}&recoemail=${recoemail}`);
                const response4 = await axios.get(`http://www.textilediwanji.com:4000/billingreport?setno=${setno}&designno=${designno}&recoemail=${recoemail}`);
    ...
    useEffect(() => {
        axios.get(`http://www.textilediwanji.com:4000/reconsilation?setno=${setno}&designno=${designno}&recoemail=${recoemail}`)
            .then(res => {
                    ...
                    <div className="container border border-1 ">
                        ...
                        <div className="row">
                            <div className="col-2 border border-1 d-flex justify-content-center align-items-center ">
                                <img src={`http://www.textilediwanji.com:4000/omkarparmaj/${maaaid}`} style={{ maxWidth: 250, maxHeight: 90 }} alt={`Image ${maaaid}`} />
                            </div>
                            <div className="col-10 border border-1">
                                {company && <h3 className="m-0 text-center">{company.companyname}</h3>}
                                {company && <p className="m-0 text-center">{company.companyaddress}</p>}
                                {company && <p className="m-0 text-center"> Email id:-{company.emailid} phone:{company.phoneno}</p>}
                                {company && <p className="m-0 text-center">GST No:- {company.gst}</p>}
                            </div>
                        </div>
                        <div className="row d-flex justify-content-evenly mt-3">
                            <div className="col-6 border border-1">
                                <div className="row bg-primary">
                                    <h4 className="text-white">Beam Inward </h4>
                                </div>
                            ...
                            <div className="col-6 border border-1">
                                <div className="row bg-primary">
                                    <h4 className="text-white">Yarn Inward</h4>
                                </div>
                                ...
                                <table className="table table-bordered mt-3 text-center">
                                    ...
                                    <tbody>
                                        {yarnparty && yarnparty.map((yarn, index) => (
                                            <tr key={index}>
                                                <td>{index + 1}</td>
                                                <td>{yarn.setNo}</td>
                                                <td>{yarn.Designno}</td>
                        ...
                        <div className="row d-flex ">
                            <div className="col-6 border border-1">
                                <div className="row bg-primary">
                                    <h4 className="text-white">Fabric outward</h4>
                                </div>
                                <table className="table table-bordered mt-3 text-center">
                                    <thead>
                                        <tr>
                                ...
                                <table className="table table-bordered text-center">
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td>{totalpick}</td>
                                            <td>{((weight1 * totalMtr) + (weight2 * totalMtr) + ((weight3 * totalMtr) ? (weight3 * totalMtr) : 0) + ((weight4 * totalMtr) ? (weight4 * totalMtr) : 0) + ((weight5 * totalMtr) ? (weight5 * totalMtr) : 0)).toFixed(2)} Kg</td>
                                        </tr>
                                    </tfoot>
                                </table>
